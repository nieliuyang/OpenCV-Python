...
$R'=R/255\\G'=G/255\\B'=B/255\\C_{max}=max(R',G',B')\\C_{min}=min(R',G',B')\\\Delta=C_{max}-C_{min}$

2. 计算

$H=\left\{\begin{matrix}0&,\Delta=0\\60\times(\frac{G'-B'}{\Delta}+0)&,C_{max}=R'\\60\times(\frac{B'-R'}{\Delta}+2)&,C_{max}=G'\\60\times(\frac{R'-G'{\Delta}+4)&,C_{max}=B'\end{matrix}\right.$
